...
void main() {
  group("small_unit_test", () {
    ...
    test("UserCheckTest", () {
      // Mockを用意する
      var ageOk = AnonymousAgeChecker(
        check: (int age) => true,
      );
      var ageNg = AnonymousAgeChecker(
        check: (int age) => false,
      );
      var addressOk = AnonymousAddressChecker(
        check: (String address) => true,
      );
      var addressNg = AnonymousAddressChecker(
    ...
    test("ApiTest", () {
      // Mockを用意する
      var itemOk = AnonymousItemChecker(
        check: (String item) => true,
      );
      var itemNg = AnonymousItemChecker(
        check: (String item) => false,
      );
      var userOk = AnonymousUserChecker(
        check: (int age, String address) => true,
      );
      var userNg = AnonymousUserChecker(
